...
def init_simulator(init_sample, energy_fn, nbrs_init, kT=297.15*quantity.kb,
                   dt=0.001, gamma=100., t_eq=2., t_total=4.):
    key = random.PRNGKey(11)
    displacement_fn, shift_fn = space.periodic_general(
        init_sample["box"], fractional_coordinates=True)
    key, split = random.split(key)
    r_init = init_sample["R"]
    init_ref_state, sim_template = sampling.initialize_simulator_template(
        simulate.nvt_langevin,
        shift_fn=shift_fn,
        nbrs=nbrs_init,
        init_with_PRNGKey=True,
        extra_simulator_kwargs={"kT": kT, "dt": dt, "gamma": gamma},
    )
    key, split = random.split(key)
    reference_state = init_ref_state(
        split, r_init,
        energy_or_force_fn=energy_fn,
        init_sim_kwargs={"mass": init_sample["mass"], 
                         "neighbor": nbrs_init,
                         "species": init_sample["species"],},
    )
    eval_timings = sampling.process_printouts(
        time_step=dt, total_time=t_total,
        t_equilib=t_eq, print_every=0.5,
    )
    trajectory_generator = sampling.trajectory_generator_init(
        sim_template, lambda _: energy_fn, eval_timings, vmap_batch=1, vmap_sim_batch=1,
    )

    return reference_state, jax.jit(trajectory_generator)
